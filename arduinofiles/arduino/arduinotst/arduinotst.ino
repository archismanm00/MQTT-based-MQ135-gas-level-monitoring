//Code taken from https://github.com/mdsiraj1992/Gassensors/blob/master/arduino%20codes/mq135_sensor/mq135_sensor.ino
//modified for my own purposes for json and mqtt
#include <ArduinoJson.h>

/************************MQ135sensor************************************/
/************************Hardware Related Macros************************************/
#define MQ135PIN (A0)                    // define which analog input channel you are going to use
...
void loop()
{
  ...
  cdoxide["gas"] = "Co2";
  cdoxide["ppm"] = MQGetGasPercentage(MQRead(MQ135PIN) / Ro, GAS_CARBON_DIOXIDE);
  JsonObject cmoxide = array.createNestedObject();
  cmoxide["gas"] = "Co";
  cmoxide["ppm"] = MQGetGasPercentage(MQRead(MQ135PIN) / Ro, GAS_CARBON_MONOXIDE);
...
int MQGetGasPercentage(float rs_ro_ratio, int gas_id)
{
  ...
  else if (accuracy == 1)
  {
    ...
    {
      return (pow(10, (1.457 * pow((log10(rs_ro_ratio)), 2) - 4.725 * (log10(rs_ro_ratio)) + 2.855))) * 100;
    }
    else if (gas_id == GAS_ALCOHOL)
    {
    ...
    {
      return (pow(10, (-1.004 * pow((log10(rs_ro_ratio)), 2) - 3.525 * (log10(rs_ro_ratio)) + 1.553))) * 1000;
    }
  }
  return 0;
}
